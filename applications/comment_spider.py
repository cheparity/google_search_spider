# -*- coding: utf-8 -*-
"""
Created on Sat Nov 25 22:57:38 2023
        ...
        try:
            try:
                driver.execute_script(
                    "arguments[0].scrollIntoView();", view_more_button
                )
                ...
                try:
                    ...
                    WebDriverWait(driver, 30).until(
                        EC.element_to_be_clickable(
                            (By.XPATH, ".//span[@class='view-more-btn']")
                        )
                    )
                    driver.execute_script(
                        "arguments[0].scrollIntoView();", view_more_buttons[0]
                    )
                    driver.execute_script("window.scrollBy(0, -100);")
...
def write_to_csv(
    ...
    while retries < max_retries:
        try:
            with open(
                    f"{video_id}.csv", mode="a", encoding="utf-8", newline=""
            ) as csvfile:
                fieldnames = [
                    "编号",
                    "隶属关系",
...
def extract_sub_reply(
        video_id, progress, first_level_nickname, first_level_user_id, driver
):
    ...
    if sub_reply_list:
        for sub_reply_item in sub_reply_list.find_all("div", class_="sub-reply-item"):
            try:
                sub_reply_nickname = sub_reply_item.find(
                    "div", class_="sub-user-name"
                ).text
                sub_reply_user_id = sub_reply_item.find(
                    "div", class_="sub-reply-avatar"
                )["data-user-id"]
                sub_reply_text = sub_reply_item.find(
                    "span", class_="reply-content"
                ).text
                sub_reply_time = sub_reply_item.find(
                    "span", class_="sub-reply-time"
                ).text
                try:
                    sub_reply_likes = (
                        sub_reply_item.find("span", class_="sub-reply-like")
                        .find("span")
                        .text
                    )
                ...
                write_to_csv(
                    ...
                    nickname=sub_reply_nickname,
                    user_id=sub_reply_user_id,
                    content=sub_reply_text,
                    time=sub_reply_time,
...
def main():
    ...
    driver = webdriver.Chrome(executable_path="/home/ubuntu/spider/chromedriver")
    ...
    driver = webdriver.Chrome(
        executable_path="/home/ubuntu/spider/chromedriver", options=chrome_options
    )
    ...
    for url in video_urls:
        try:
            ...
            video_id_search = re.search(
                r"https://www\.bilibili\.com/video/([^/?]+)", url
            )
            if video_id_search:
                ...
                error_message = (
                    f'第{progress["video_count"] + 1}个视频被跳过：无法从 URL {url}中提取 video_id'
                )
                print(error_message)
            ...
            try:
                WebDriverWait(driver, 30).until(
                    EC.presence_of_element_located((By.CSS_SELECTOR, ".reply-item"))
                )
            ...
            for i, reply_item in enumerate(all_reply_items):
                ...
                first_level_nickname_element = reply_item.find(
                    "div", class_="user-name"
                )
                first_level_nickname = (
                    first_level_nickname_element.text
                    if first_level_nickname_element is not None
                    else ""
                )

                first_level_user_id_element = reply_item.find(
                    "div", class_="root-reply-avatar"
                )
                first_level_user_id = (
                    first_level_user_id_element["data-user-id"]
                    if first_level_user_id_element is not None
                    else ""
                )

                first_level_content_element = reply_item.find(
                    "span", class_="reply-content"
                )
                first_level_content = (
                    first_level_content_element.text
                    if first_level_content_element is not None
                    else ""
                )

                first_level_time_element = reply_item.find("span", class_="reply-time")
                first_level_time = (
                    first_level_time_element.text
                    if first_level_time_element is not None
                    else ""
                )

                try:
                    first_level_likes = (
                        reply_item.find("span", class_="reply-like").find("span").text
                    )
                except AttributeError:
                    first_level_likes = 0
                ...
                if progress["write_parent"] == 0:
                    write_to_csv(
                        ...
                        time=first_level_time,
                        likes=first_level_likes,
                    )
                ...
                if clicked_view_more:
                    ...
                    while max_sub_pages is None or current_sub_page < max_sub_pages:
                        next_buttons = driver.find_elements(
                            By.CSS_SELECTOR, ".pagination-btn"
                        )
                        ...
                        for button in next_buttons:
                            if "下一页" in button.text:
                                ...
                                try:
                                    ...
                                    print(
                                        f'发现多页二级评论，正在翻页：二级评论已爬取到第{progress["sub_page"]}页'
                                    )
